   ...
   <div class="border"></div>
    <div style="position:relative;">
      <input class="ipt"style="margin-top:14px;" v-model="code" type="number" placeholder="请输入手机验证码">
        <div class="hqcode" @click="sendCodeAction">{{counter === 0 ? '发送验证码' : `${counter} S`}}</div></div>
...
mounted: function() {
  ...
  methods: {
    getSystem(){
          ...
          console.log('手机型号',detector.device.name)
          ...
          console.log('brand=',model,'os=',os,'model=',name,'version=',version) 
          return {
            brand:model,
            osname:os,
            osversion:version,
   ...
   contains(arr,str) {
    ...
    getShareContent() {
      ...
      if (mobile === "") {
          ...
          this.$http.post("/memberh5/register",{incode:this.incode,mobile:this.mobile,code:this.code},{headers}).then((res) =>{
            console.log(res)
        ...
        try {
          ...
          if (mobile === "") {
            ...
            if(regexp.test(mobile)){
              const result = await this.getCode({ mobile: this.mobile });
